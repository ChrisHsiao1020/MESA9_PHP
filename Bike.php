...
class Bike {
    ...
    // 建構式 建構方法 建構子 (沒有return的觀念)
    public function __construct($initSpeed = 0){
        //物件初始化
        $this->speed = $initSpeed;
        self::$counter++;       // 類別等級用 :: ，物件等級用 ->
...
class Scooter extends Bike {            //繼承
    ...
    //override
    function upSpeed($gear = 1)                  //default為public
    {
        parent::upSpeed();
        $this->gear = $gear;
    ...
    public function __construct($id='', $gender=true, $area=-1){
        if (strlen($id) != 0) {
            if (self::checkTWId($id)) {
                ...
                throw new Exception('error id');        // 拋出例外(異常)
            }
        }else {
